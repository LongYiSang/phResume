...
1.  **项目结构：**
    * 初始化 Git 仓库。
    * 创建顶层目录：`/backend`, `/frontend`, `/docker`。
2.  **Docker 编排 (`docker-compose.yml`)：**
    * 定义所有**开发期**服务：
        * `api` (后端 Go)
        * `frontend` (Next.js)
        * `db` (使用 `postgres:15-alpine` 镜像)
        * `nginx` (使用 `nginx:alpine` 镜像)
    * 配置服务间的 `depends_on` 和共享网络。
3.  **Dockerfiles：**
    * `/backend/Dockerfile`: 基于 `golang:alpine`，设置工作目录，拷贝 `go.mod` 并下载依赖。
    * `/frontend/Dockerfile`: 基于 `node:alpine`，设置 `package.json` 并安装 `npm` 依赖。
4.  **配置 (`Viper`)：**
    * 在 `backend` 中集成 `Viper`。
    * 使其能够从**环境变量**（由 `docker-compose.yml` 注入）中读取配置（如数据库连接字符串）。
5.  **反向代理 (`nginx`)：**
    * 配置 `nginx.conf` 作为所有流量的入口。
    * `location /` 转发到 `frontend:3000`。
    * `location /api/` 转发到 `api:8080` (Gin)。
    * **关键：** 这一步能从根本上解决开发中的 `CORS` 跨域问题。

**验收标准：**
* 在本地运行 `docker-compose up --build`。
* 访问 `http://localhost` 能看到 Next.js 的默认页面。
* 访问 `http://localhost/api/health` 能收到 Gin 返回的 `{"status": "ok"}` JSON。
...
1.  **`go-rod` 攻坚（Spike）：**
    * **关键：** 修改 `/backend/Dockerfile`，在 `golang:alpine` 镜像中**安装 `chromium` 及其所有图形/字体依赖库**。
    * 编写一个临时的 `main.go`，用 `go-rod` 将一个写死的 HTML 字符串 (`<h1>Hello</h1>`) 保存为 `test.pdf`。
    * 运行 `docker-compose run --rm api go run main.go` 确保 PDF 能够成功生成。
2.  **后端 API (同步)：**
    * 使用 `GORM` 定义 `Resume` 和 `User` 结构体。
    * 在 Gin 启动时运行 `gorm.AutoMigrate`。
    * 实现 **`POST /api/v1/resume`**：(暂时无需认证) 接收 JSON，存入 `db`。
    * 实现 **`GET /api/v1/resume/:id/download`**：
...
1.  **身份认证 (API)：**
    * 实现用户注册 `POST /api/v1/auth/register`。
    * 实现登录 `POST /api/v1/auth/login` (返回 `JWT` 和 `Refresh Token`)。
    * 实现 `POST /api/v1/auth/refresh` (刷新令牌)。
    * 实现 `POST /api/v1/auth/logout` (利用 Redis 实现 Refresh Token 吊销)。
