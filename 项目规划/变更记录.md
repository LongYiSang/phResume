...
- 修复打印页图片渲染崩溃：`ImageItem` 在 `preSignedURL/dataURI` 已存在时不再依赖 `AuthProvider`（避免 `/print*` 绕开 `AuthProvider` 后 `useAuth()` 直接 throw，导致 `#pdf-render-ready` 永远不出现并触发 worker 超时重试）：`frontend/components/ImageItem.tsx (line 118)`。
